...
# promox variables
promox_ve_ver="7.3-1"
promox_ve_url_domain="http://download.proxmox.com"
promox_ve_url_path="/iso/"
promox_ve_url_base="$promox_ve_url_domain$promox_ve_url_path"
promox_ve_url_page="proxmox-ve_$promox_ve_ver.iso"
promox_ve_url_iso_sha256sums="SHA256SUMS"
...
if validate_url $promox_ve_url_base$promox_ve_url_page; then
    # Get page and check if file already exists in current working directory.
    wget --no-clobber $promox_ve_url_base$promox_ve_url_page
    else
    # Print error message.
...
extracted_iso_shasum=$(curl -s $promox_ve_url_base$promox_ve_url_iso_sha256sums |\
grep "$promox_ve_url_page" |\
cut -f 1 -d ' ')
...
case $REPLY in
    y | Y)
    echo "Copy and convert /dev/$disk_target with $promox_ve_url_page "
    ;;
    * )
    echo "Aborting..."
    exit
...
if create_usb $disk_target; then
    ...
    # Print error message.
    echo "/dev/$disk_target failed imaged with $promox_ve_url_page Aborting..." && exit 1
fi

# Print success or exit for any errors.
if [ $? -eq 0 ]
then
    echo "Successfully prepared Proxmox Version: $promox_ve_ver"
else
    echo "Could not prepare Proxmox Version: $promox_ve_ver" >&2
fi
